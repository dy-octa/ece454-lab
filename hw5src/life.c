 ...
 * Active list: 'Midactives' are actually stored.
 * Each element stores the col, record of a row starts with negative elements -row-1
 * Note that a 'midactive' can be -1 or 1024, which represents that they are actually on 1023 or 0, and may affect 0 or 1023
 * -1 must be distinguished by row prologue of row #0
...
/*****************************************************************************
 * Helper function: check validity of change / active list
 ****************************************************************************/
...
/*****************************************************************************
 * Helper function: check validity of border change list
 ****************************************************************************/
...
/*****************************************************************************
 * Helper function: output a change / active list
 ****************************************************************************/
...
/*****************************************************************************
 * Helper function: Output a border change list
 ****************************************************************************/
...
/*****************************************************************************
 * Helper function: Print current alive cells for debugging
 ****************************************************************************/
...
//		printf(" ");
	...
	for (i = 1; i < active_cnt; i += (offset == 1)) {
		if (active_list[i] < -1) { // A new row in active list
			if (newrow)
				--change_cnt;
			row = -active_list[i++] - 1;
			change_list[change_cnt++] = -row - 1;
		...
		if (offset == 1) // Midactives are actually stored, so a col in list may represent col-1, col and col+1
		...
		// Update the board, border buffers and change lists
	...
	// If the last row in active list doesn't produce any changes
	if (newrow && i != 1)
		--change_cnt;
 ...
 * Deprecated in current version of code: midactives are used directly in board_step
 ****************************************************************************/
...
//			printf(" ");
		...
		// Decide next row which may contain midactives
	...
	if (up != i && -change_list[up] - 1 >= row - 1
	    && -change_list[up] - 1 <
	       srows + BLOCKHEIGHT - 2) { // Rows influenced by left and right border should be already used up
...
void *thread_handler_generic(void *thread_args) {
	...
	thread_board_generic(outboard, inboard, srows, scols, nrows, nrowsmax, ncols, ncolsmax, gens_max, mutex, cond,
	                     done);
	return NULL;
}
